...
Run Spotify Smart Agent directly in your terminal with a beautiful text-based UI!
...
[number]                      → Jump to track # in current album
                                (e.g., "5" plays track #5)
...
**`startup_terminal.bat`**
```batch
@echo off
start "Spotify Agent" python C:\Users\liwenj\spotify_agent\spotify_agent_terminal.py --quit-mode resume
```

Add to Windows Startup folder:
```
Win+R → shell:startup → paste startup_terminal.bat
```

### Remote Access

Run on server, control via SSH:
```bash
ssh user@server
cd spotify_agent
python spotify_agent_terminal.py
...
```batch
@echo off
python spotify_agent_terminal.py --start-mode resume --quit-mode resume
```
